(function(){
  ...
  let currentOrder=null; let busy=false; let ordersMeta=null;
  ...
  function norm(response){
    if(Array.isArray(response)) return { items: response, meta: null };
    if(Array.isArray(response?.data)) return { items: response.data, meta: response?.meta || null };
    if(Array.isArray(response?.items)) return { items: response.items, meta: response?.meta || null };
    if(Array.isArray(response?.data?.items)) return { items: response.data.items, meta: response?.data?.meta || response?.meta || null };
    return { items: [], meta: response?.meta || response?.data?.meta || null };
  }
  async function load(){body.innerHTML='<tr><td colspan="10" class="empty">Cargando pedidos…</td></tr>'; setMsg(msg,''); try{const data=await req('/api/admin/orders'); const { items, meta }=norm(data); ordersMeta=meta; if(!items.length){body.innerHTML='<tr><td colspan="10" class="empty">Sin pedidos.</td></tr>';return;} body.innerHTML=items.map(o=>`<tr><td>${txt(o.id)}</td><td>${fmt(o.createdAt)}</td><td>${chip(o.status)}</td><td>${money(o.total)}</td><td>${txt(o.user?.email||o.email,'—')}</td><td>${txt(o.shippingCarrier,'—')}</td><td>${o.trackingNumber?txt(o.trackingNumber):'Sin tracking'}</td><td>${fmt(o.shippedAt)||'No enviado todavía'}</td><td>${fmt(o.deliveredAt)||'No entregado todavía'}</td><td><button class="btn" data-order-id="${o.id}">Gestionar</button></td></tr>`).join('');}catch(e){setMsg(msg,e.message||'Error cargando pedidos','error'); body.innerHTML='<tr><td colspan="10" class="empty">No se pudo cargar.</td></tr>';}}
