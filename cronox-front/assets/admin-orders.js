(function(){
  ...
  function norm(response){ if(Array.isArray(response)) return { items: response }; if(Array.isArray(response?.data)) return { items: response.data }; if(Array.isArray(response?.items)) return { items: response.items }; if(Array.isArray(response?.data?.items)) return { items: response.data.items }; return { items: [] }; }
  function renderEmpty(email){const text=email?`No hay pedidos para ${txt(email)}.`:'Sin pedidos.'; body.innerHTML=`<tr><td colspan="10" class="empty">${text}</td></tr>`;}
  async function load(){body.innerHTML='<tr><td colspan="10" class="empty">Cargando pedidos…</td></tr>'; setMsg(msg,''); try{const email=getEmailFilter(); const query=email?`?email=${encodeURIComponent(email)}`:''; const data=await req(`/api/admin/orders${query}`); const { items }=norm(data); if(!items.length){renderEmpty(email); return;} body.innerHTML=items.map(o=>`<tr><td>${txt(o.id)}</td><td>${fmt(o.createdAt)}</td><td>${chip(o.status)}</td><td>${money(o.total)}</td><td>${txt(o.userEmail||o.user?.email||o.email,'—')}</td><td>${txt(o.shippingCarrier,'—')}</td><td>${o.trackingNumber?txt(o.trackingNumber):'Sin tracking'}</td><td>${fmt(o.shippedAt)||'No enviado todavía'}</td><td>${fmt(o.deliveredAt)||'No entregado todavía'}</td><td><button class="btn" data-order-id="${o.id}">Gestionar</button></td></tr>`).join('');}catch(e){setMsg(msg,e.message||'Error cargando pedidos','error'); body.innerHTML='<tr><td colspan="10" class="empty">No se pudo cargar.</td></tr>';}}
  ...
  emailSearch?.addEventListener('keydown',(e)=>{if(e.key==='Enter'){e.preventDefault(); load();}});
