...
- `/launch.html`: one-use login token, hashed in the database, valid for 24 hours. A second click on the landing page protects against email link scanners consuming the token. The token stays in the URL fragment and is removed before requests. Standard session cookies are issued. This email grants access to the account and must not be forwarded. Admin accounts are excluded.
